...
const deleteKeyByValue = (obj, value) =>
...
exports.receivers = (io) => {
  io.on("connection", function (socket) {
    socket.on(LOGIN, async function (username) {
      let user_id = await getUserInfo(username)
        .then((data) => data[0].id)
        .catch((error) => error);
      await updateConnection("last_connection", new Date(Date.now()), username)
        .then((data) => {
          if (data.affectedRows === 0) {
            io.to(socket.id).emit(NOTIFICATION, `${username} not found`);
          } else {
            userRegister[username] = { socket: socket.id, user_id: user_id };
    ...
    socket.on(CHAT, async function (username, user_target, msg) {
      let user_target_id = await await getUserInfo(user_target)
        .then((data) => data[0].id)
        .catch((error) => error);
      if (userRegister[username]) {
        ...
        let conversationId = await getConversationId(
          userRegister[username].user_id,
          user_target_id
        )
          .then((data) => data[0].conversation_id)
          .catch((error) => {
        ...
        if (userRegister[user_target]) {
          let blocks = await getBlock(userRegister[username].user_id).then(
            (data) => data
          );
          ...
          blocks = await getBlock(userRegister[user_target].user_id).then(
            (data) => data
          );
          for (let ind = 0; ind < blocks.length; ind += 1) {
            let blocked_id = blocks[ind].user_blocked;
          ...
          io.to(userRegister[user_target].socket).emit(NOTIFICATION, {
            message: `You received a message from ${username}`,
            id: id,
          });
          io.to(userRegister[user_target].socket).emit(
            CHAT,
            `${username}: ${msg}`
          );
        }
        if (userRegister[username]) {
          io.to(userRegister[username].socket).emit(
            CHAT,
            `${username}: ${msg}`
          );
        }
    ...
    socket.on(LIKE, async function (username, user_liked) {
      ...
      if (userRegister[user_liked]) {
        ...
        blocks = await getBlock(userRegister[user_liked].user_id).then(
          (data) => data
        );
        for (let ind = 0; ind < blocks.length; ind += 1) {
          let blocked_id = blocks[ind].user_blocked;
        ...
        io.to(userRegister[user_liked].socket).emit(NOTIFICATION, {
          message: `user ${username} likes you`,
          id: id,
        });
      }
    });
    socket.on(LIKEBACK, async function (username, user_liked) {
      ...
      await addNotification(
        id,
        user_liked,
        `user ${username} likes you back, it's a match`
      );
      if (userRegister[user_liked]) {
        let blocks = await getBlock(userRegister[username].user_id).then(
          (data) => data
        ...
        io.to(userRegister[user_liked].socket).emit(NOTIFICATION, {
          message: `you are matched with ${username}`,
          id: id,
        });
      }
      if (userRegister[username]) {
        ...
        io.to(userRegister[username].socket).emit(NOTIFICATION, {
          message: `match with ${user_liked}`,
          id: id,
        });
      }
    });
    socket.on(VIEW, async function (username, user_viewed) {
      ...
      await addNotification(
        id,
        user_viewed,
        `user ${username} viewed your profile`
      );
      if (userRegister[user_viewed]) {
        ...
        blocks = await getBlock(userRegister[user_viewed].user_id).then(
          (data) => data
        );
        for (let ind = 0; ind < blocks.length; ind += 1) {
          let blocked_id = blocks[ind].user_blocked;
        ...
        io.to(userRegister[user_viewed].socket).emit(NOTIFICATION, {
          message: `user ${username} viewed ${user_viewed}`,
          id: id,
        });
      }
    });
    socket.on(DISLIKE, async function (username, user_viewed) {
      ...
      if (userRegister[user_viewed]) {
        ...
        io.to(userRegister[user_viewed].socket).emit(NOTIFICATION, {
          message: `${username} doesn't like you anymore`,
          id: id,
        });
      }
    });
