...
const User = {
  ...
  addUser: async (req, res) => {
    ...
    if (!(username && name && surname && email && password)) {
      return res
        .status(400)
        .json({client: "Missing informations, fill the form"});
    }
    ...
    if (!PASSWORD_REGEX.test(password) || password.length < 8) {
      return res.status(400).json({
        client:
          "Invalid password, it should contain at least one capital letter, one numerical character and a minimun of 8 characters.",
      });
    }
    if (!USERNAME_REGEX.test(username) || username.length < 6) {
      return res.status(400).json({
        client:
          "Invalid username, it should contain only letters, numbers and a minimun of 6 characters",
      });
    }
    if (!NAME_REGEX.test(name) || name.length < 2) {
      return res
        .status(400)
        .json({client: "Invalid name, it should contain only letters"});
    }
    if (!NAME_REGEX.test(surname) || surname.length < 2) {
      return res.status(400).json({
        client:
          "Invalid surname, it should contain only letters and it should be longer that 2 characters",
      });
    }
    try {
      ...
      mail(
        email,
        "activation link matcha",
        null,
        '<p>lien pour activer votre compte : <a href="http://localhost:3000/activate?id=' +
          confirmation +
          '"> lien pour activer </a></p>'
      );
    } catch (error) {
  ...
  resetPasswordEmail: async (req, res) => {
    ...
    resultJson = JSON.parse(resultJson);
    if (resultJson[0]) {
      await updateFieldUser("password_reset", 1, resultJson[0].id).catch((err) => {
        console.log(err);
      ...
      mail(
        ...
        "reset link matcha",
        null,
        '<p>lien pour changer votre mot de passe : <a href="http://localhost:3000/password?id=' +
          resultJson[0].confirmation +
          "&username=" +
          resultJson[0].username +
          '"> lien reset </a></p>'
      );
  ...
  isPasswordReset: async (req, res) => {
    ...
    if (resultJson[0].nb !== 1) {
      return res
        .status(401)
        .json({client: "Contact the website administrator"});
    }
